				...
				const u_int dataOffset, std::vector<double> &dataMI)
{
	...
	for (auto b1id = block[0]; b1id <= block[1]; ++b1id)
	{
		...
		for (u_int b2id = b2_s; b2id < b2_e; ++b2id)
		{
			...
			dataMI[dataIdx] = S1[id2indx[tmp_d1]] + S1[id2indx[tmp_d2]] - dataIn[dataIdx];
			++dataIdx;
		}
	}
}
				...
				const std::vector<double> &dataIn, std::vector<double> &dataMI)
{
	...
	for (auto ix = 0u, iv = 0u; iv < index2id_2d.size(); ix += 1, iv += 2)
	{
		dataMI[ix] = S1[id2index_1d1.at(index2id_2d[iv])] + S2[id2index_1d2.at(index2id_2d[iv + 1])] - dataIn[ix];
	}
}
					 ...
					 std::vector<double> &mi_data)
{
	...
	for (auto b1id = block[0]; b1id <= block[1]; ++b1id)
	{
		...
		{
			u_int tmp_d2 = b_neigh[b2id];
			mi_data[dataIdx] = S1d1[id2indx1[tmp_d1]] + S1d2[id2indx2[tmp_d2]] - dataIn[dataIdx];
			++dataIdx;
		}
	}
}
