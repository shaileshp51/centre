...
#ifdef DEBUG_CODE
			std::cout << "Succesfully read " << str_dim_type
					<< "-int data for did(" << bl_strat_id + idx << ", "
					<< dim_id << ")" << std::endl;
#endif
		}
...
#pragma omp parallel for
		for (int idx = 0; idx < block_tasks; ++idx) {
			...
			if (binData1D(nsteps, bin_schemes, nframes_tot_eff,
				...
				mprinterr("Error: Binning %s data bond id(%d)",
		...
		if (isWritefreq && (frqWriteset & bat_type)) {
			if (d1DHist->writeRecords(bingrp_v) != 0) {
				mprinterr("Error: Writing binned data for %s id(%d, %d)",
...
void EntropyCalculator::run2d_xx(BATSet dim_type, int num_threads) {
	...
	// std::vector<u_int> proc_rids(numprocs,
	...
	for (u_int bi = 0; bi < num_dimTypeKeys; bi += chached_dims) {
		const int bl_rows =
				(bi + chached_dims <= num_dimTypeKeys) ?
						chached_dims : (num_dimTypeKeys - bi);
		...
		for (auto rno = 0; rno < bl_rows; ++rno) {
			...
			if (intXX2DTraj->readCoords(dtyp_id1, 0, n_schemes, crd_rows[rno])
					!= 0) {
				mprinterr("Error: Reading int %s for row id(%d)",
			...
			if (block_tasks) {
				...
				for (std::map<u_int, u_int>::iterator it =
					...
					if (intXX2DTraj->readCoords(it->first, 0, n_schemes,
						...
						mprinterr("Error: Reading int of %s for column id(%d)",
...
#pragma omp parallel for
					...
					for (auto const cj : neigh) {
						if (cj >= dimtypes_v[bj]
							...
							if (binData2D(nsteps, bin_schemes, nframes_tot_eff,
								...
								mprinterr("Error: Bining data %s id(%d, %d)",
				...
				if (isWritefreq && (frqWriteset & BATSet::BB2D)) {
					if (xx2DHist->writeRecords(bingrp_v) != 0) {
						mprinterr(
								"Error: Writing histogram bin data for %s id(%d, %d)",
