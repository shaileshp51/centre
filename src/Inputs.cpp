...
bool Inputs::Init(const std::string &inpFileName) {
	...
	if (Utils::fileExists(inpFileName)) {
		...
		for (auto x : symbolTable) {
			...
			} else if (tagO == "discretization.optimizedih") { // optimizedih
		...
		if (bats.getFnames().size() != bats.getNframes().size()) {
			...
			for (u_int i = 0; i < bats.getFnames().size(); ++i) {
				...
				if (fltype == NetcdfFile::NCTYPE::NC_CENTRETRAJ) {
					...
					} else if (bats.getNbond() != cInfo.nBonds()) {
						mprinterr(
					...
					} else if (bats.getNangle() != cInfo.nAngles()) {
						mprinterr(
					...
					} else if (bats.getNdihed() != cInfo.nDihedrals()) {
						mprinterr(
		...
		if (entropy.getNumframe() > 0
				&& (entropy.getStartframe() > 0 || entropy.getStrideframe() > 1)) {
			nframes_eff = (entropy.getNumframe() - entropy.getStartframe()
					+ entropy.getStrideframe() - 1) / entropy.getStrideframe();
		} else {
		...
		if (isOK && entropy.isUseSubset()) {
			std::string fname = control.getInfilepath() + "/"
					+ entropy.getSubsetfile();
			if (isOK && Utils::fileExists(fname)) {
				...
				if (subset.isGood() && entropy.isUseNeighbor()) {
					std::string fn = control.getInfilepath() + "/"
...
//			<< std::endl;
	...
	for (int est = 0; est < inp.getEstimators().size(); ++est) {
		if (f != 0)
			os << ", ";
	...
	if (inp.getEntropy().getNumframe() != nframe_bats
			|| (inp.getEntropy().getStrideframe() != 1)
			|| (inp.getEntropy().getStartframe() != 0)) {
