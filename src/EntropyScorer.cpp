...
void EntropyScorer::run()
{
	...
	if (inputs.getControl().isGenConvgdata())
	{
		...
		vector<ull_int> dim_lens(2, n_bnd_eff);
		...
		vector<u_int> index2id_bb(2*n_bnd2d);
		vector<u_int> index2id_ba(2*n_ba2d);
		vector<u_int> index2id_bd(2*n_bd2d);
		vector<u_int> index2id_aa(2*n_ang2d);
		vector<u_int> index2id_ad(2*n_ad2d);
		vector<u_int> index2id_dd(2*n_dih2d);
		...
		if (inputs.getEntropy().getWorkSet() & BATSet::BB2D)
		{
			...
			//index2id_bb.reserve(2 * n_bnd2d);
			for (size_t itmp = 0; itmp < ids_bb2.size(); ++itmp)
			{
				index2id_bb[itmp] = ids_bb2[itmp];
			}
		}
		...
		if (inputs.getEntropy().getWorkSet() & BATSet::BA2D)
		{
			...
			//index2id_ba.reserve(2 * n_ba2d);
			for (size_t itmp = 0; itmp < ids_ba2.size(); ++itmp)
			{
				index2id_ba[itmp] = ids_ba2[itmp];
			}
		}
		...
		if (inputs.getEntropy().getWorkSet() & BATSet::BD2D)
		{
			...
			//index2id_bd.reserve(2 * n_bd2d);
			for (size_t itmp = 0; itmp < ids_bd2.size(); ++itmp)
			{
				index2id_bd[itmp] = ids_bd2[itmp];
			}
		}
		...
		if (inputs.getEntropy().getWorkSet() & BATSet::AA2D)
		{
			...
			// index2id_aa.reserve(2 * n_ang2d);
			for (size_t itmp = 0; itmp < ids_aa2.size(); ++itmp)
			{
				index2id_aa[itmp] = ids_aa2[itmp];
			}
		}
		...
		if (inputs.getEntropy().getWorkSet() & BATSet::AD2D)
		{
			...
			//index2id_ad.reserve(2 * n_ad2d);
			for (size_t itmp = 0; itmp < ids_ad2.size(); ++itmp)
			{
				index2id_ad[itmp] = ids_ad2[itmp];
			}
		}
		...
		if (inputs.getEntropy().getWorkSet() & BATSet::DD2D)
		{
			...
			//index2id_dd.reserve(2 * n_dih2d);
			for (size_t itmp = 0; itmp < ids_dd2.size(); ++itmp)
			{
				index2id_dd[itmp] = ids_dd2[itmp];
			}
		}
		...
		for (hbin_t estid = 0; estid < nestimators; ++estid)
		{
			...
			for (hbin_t b_sch = 0; b_sch < n_schemes; ++b_sch)
			{
				...
				double mist_bb = 0.0, mist_aa = 0.0, mist_dd = 0.0, mist_ba = 0.0, mist_bd = 0.0, mist_ad = 0.0, mist_doublet;
            ...
            */
					...
					doublet = singlet - mi_bb - mi_aa - mi_dd - mi_ba - mi_bd - mi_ad;
					int fw = 13;
					if (stp_id == 0)
					{
						string fname = inputs.getControl().getOutfilepath() + "MIE" + "-" + getEstimatorName(est) + "_convergence_b-" + to_string((int)bin_schemes[b_sch]) + "." + inputs.getEntropy().getConvergenceFileExt();
					...
					if (inputs.getMethod() == EntropyMethods::MIST)
					{
						...
						// from 0
						if ((inputs.getEntropy().getWorkSet() & BATSet::BB2D) || (inputs.getEntropy().getWorkSet() & BATSet::BA2D) || (inputs.getEntropy().getWorkSet() & BATSet::BD2D))
						{
						...
						// from n_bnd_eff
						if ((inputs.getEntropy().getWorkSet() & BATSet::AA2D) || (inputs.getEntropy().getWorkSet() & BATSet::AD2D))
						{
						...
						if ((inputs.getEntropy().getWorkSet() & BATSet::BB2D))
						{
							for (auto ix = 0u, iv = 0u; iv < index2id_bb.size(); iv += 2, ix += 1)
							{
								Edge edg(id2index_b[index2id_bb[iv]],
										 id2index_b[index2id_bb[iv + 1]],
										 mi_bbv[ix]);
						...
						if ((inputs.getEntropy().getWorkSet() & BATSet::BA2D))
						{
							for (auto ix = 0u, iv = 0u; iv < index2id_ba.size(); iv += 2, ix += 1)
							{
								Edge edg(id2index_b[index2id_ba[iv]],
										 id2index_a[index2id_ba[iv + 1]] + ang_node_id_offset,
										 mi_bav[ix]);
								graph.edges.push_back(edg);
							}
						}
						...
						if ((inputs.getEntropy().getWorkSet() & BATSet::BD2D))
						{
							for (auto ix = 0u, iv = 0u; iv < index2id_bd.size(); iv += 2, ix += 1)
							{
								Edge edg(id2index_b[index2id_bd[iv]],
										 id2index_d[index2id_bd[iv + 1]] + dih_node_id_offset,
										 mi_bdv[ix]);
								graph.edges.push_back(edg);
							}
						}
						if ((inputs.getEntropy().getWorkSet() & BATSet::AA2D))
						{
							for (auto ix = 0u, iv = 0u; iv < index2id_aa.size(); iv += 2, ix += 1)
							{
								Edge edg(
									id2index_a[index2id_aa[iv]] + ang_node_id_offset,
									id2index_a[index2id_aa[iv + 1]] + ang_node_id_offset,
									mi_aav[ix]);
								graph.edges.push_back(edg);
							}
						}
						...
						if ((inputs.getEntropy().getWorkSet() & BATSet::AD2D))
						{
							for (auto ix = 0u, iv = 0u; iv < index2id_ad.size(); iv += 2, ix += 1)
							{
								Edge edg(
									id2index_a[index2id_ad[iv]] + ang_node_id_offset,
									id2index_d[index2id_ad[iv + 1]] + dih_node_id_offset,
									mi_adv[ix]);
								graph.edges.push_back(edg);
							}
						}
						...
						if ((inputs.getEntropy().getWorkSet() & BATSet::DD2D))
						{
							for (auto ix = 0u, iv = 0u; iv < index2id_dd.size(); iv += 2, ix += 1)
							{
								Edge edg(
									id2index_d[index2id_dd[iv]] + dih_node_id_offset,
									id2index_d[index2id_dd[iv + 1]] + dih_node_id_offset,
									mi_ddv[ix]);
								graph.edges.push_back(edg);
							}
						}
						...
						ofstream treefile;
						string fmisttree = inputs.getControl().getOutfilepath() + "Tree-MIST" + "-" + getEstimatorName(est) + "_b-" + to_string((int)bin_schemes[b_sch]) + "_s-" + to_string((int)stp_id) + "." + inputs.getEntropy().getConvergenceFileExt();
						...
						treefile.close();
						mist_doublet = 0.0, mist_bb = 0.0, mist_ba = 0.0, mist_bd = 0.0;
						mist_aa = 0.0, mist_ad = 0.0, mist_dd = 0.0;
						for (u_int t = 0; t < mistree.size(); t++)
						{
							Edge e = mistree[t];
							if (graph.nodes[e.source].type == BAT_t::BOND && graph.nodes[e.dest].type == BAT_t::BOND)
							{
								mist_bb += e.weight;
							}
							else if ((graph.nodes[e.source].type == BAT_t::BOND && graph.nodes[e.dest].type == BAT_t::ANGLE) || (graph.nodes[e.source].type == BAT_t::ANGLE && graph.nodes[e.dest].type == BAT_t::BOND))
							{
								mist_ba += e.weight;
							}
							else if ((graph.nodes[e.source].type == BAT_t::BOND && graph.nodes[e.dest].type == BAT_t::DIHEDRAL) || (graph.nodes[e.source].type == BAT_t::DIHEDRAL && graph.nodes[e.dest].type == BAT_t::BOND))
							{
								mist_bd += e.weight;
							}
							else if (graph.nodes[e.source].type == BAT_t::ANGLE && graph.nodes[e.dest].type == BAT_t::ANGLE)
							{
								mist_aa += e.weight;
							}
							else if ((graph.nodes[e.source].type == BAT_t::ANGLE && graph.nodes[e.dest].type == BAT_t::DIHEDRAL) || (graph.nodes[e.source].type == BAT_t::DIHEDRAL && graph.nodes[e.dest].type == BAT_t::ANGLE))
							{
								mist_ad += e.weight;
							}
							else if (graph.nodes[e.source].type == BAT_t::DIHEDRAL && graph.nodes[e.dest].type == BAT_t::DIHEDRAL)
							{
								mist_dd += e.weight;
							}
						}
						mist_doublet = singlet - mist_bb - mist_aa - mist_dd - mist_ba - mist_bd - mist_ad;
						int fw = 13;
						if (stp_id == 0)
						{
							string fname2 =
								inputs.getControl().getOutfilepath() + "MIST" + "-" + getEstimatorName(est) + "_convergence_b-" + to_string((int)bin_schemes[b_sch]) + "." + inputs.getEntropy().getConvergenceFileExt();
