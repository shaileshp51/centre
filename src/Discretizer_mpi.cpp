								...
								const int n_thread_perproc)
{
	if (rank == MASTER_PROC)
	{
		...
		Netcdf_BAT traj_inp;
		std::string fname = inputs.getControl().getInfilepath() + inputs.getBats().getFnames()[0];
		...
		{
			int block_tasks =
				(bl_strat_id + chached_dims <= (int)n_dtyptasks) ? chached_dims : (n_dtyptasks - bl_strat_id);
			...
			{
				int trjreadstatus = 0;
				auto read_start_id = bl_strat_id + (process_idx - 1) * curr_cache_per_proc;
				...
				{
					LOG_ERROR(
						"Rank[%d]>> Reading real-data for bonds-id range (%d, %d)",
				...
				auto _durcomm1 = std::chrono::duration_cast<
									 ClockResolution>(_endcomm1 - _endr)
									 .count();
				dscrt_dtyp->curr_comm += ClockResolution(_durcomm1);
				progressstate.dscrt.curr_comm += ClockResolution(_durcomm1);
			}
			MPI_Barrier(MPI_COMM_WORLD);
			...
			auto n_tsk4m_or_1 = (n_task4master > 0 ? n_task4master : 1);
			std::vector<std::vector<hbin_t>> dtypes_int_v_master(
				n_tsk4m_or_1,
				std::vector<hbin_t>(bin_schemes.size() * nfrm4rl2int));
			std::vector<std::vector<double>> dtypes_extrm_v_master(
				n_tsk4m_or_1, std::vector<double>(4, 0.0));
			std::vector<hbin_t> nfoundmins_master(n_tsk4m_or_1, 0);
			std::vector<std::vector<double>> min_poss_master(
				n_tsk4m_or_1,
				std::vector<double>(inputs.getVmkde().getNmaxconf(), 0.0));
			std::vector<CoordBAT> vecs_CoordBAT_master(
				n_tsk4m_or_1, CoordBAT(-1, bin_schemes.size(), nfrm4rl2int));
			if (n_task4master > 0)
			{
				...
				{
					LOG_ERROR(
						"Rank[%d]>> Reading real-data for bond-ids range (%d, %d)",
						rank, bl_strat_id, bl_strat_id + block_tasks);
					exit(0);
				}
				auto _endr = Clock::now();
...
#pragma omp parallel for
				for (int tidx = 0; tidx < n_task4master; ++tidx)
				{
					...
					else if (inputs.getBats().getPdfmethod() == PDFMethod::vonMisesKDE)
					{
					...
					(vecs_CoordBAT_master[tidx]).setCoords(dtypes_extrm_v_master[tidx][0], dtypes_extrm_v_master[tidx][1], dtypes_extrm_v_master[tidx][2], dtypes_extrm_v_master[tidx][3], 0, (hbin_t)bin_schemes.size(), 0, nfrm4rl2int - 1, dtypes_int_v_master[tidx]);
				}

				auto _endcomm2 = Clock::now();
				auto _durcomm2 = std::chrono::duration_cast<
									 ClockResolution>(_endcomm2 - _endr)
									 .count();
				 ...
				 ++process_idx)
			{
				...
				for (auto recv_idx = 0; recv_idx < curr_cache_per_proc;
					 ++recv_idx)
				{
					size_t did = bl_strat_id + (process_idx - 1) * curr_cache_per_proc + recv_idx;
					...
					(vecs_CoordBAT[recv_idx]).setCoords(dtypes_extrm_v[recv_idx][0], dtypes_extrm_v[recv_idx][1], dtypes_extrm_v[recv_idx][2], dtypes_extrm_v[recv_idx][3], 0, (hbin_t)bin_schemes.size(), 0, nfrm4rl2int - 1, dtypes_int_v[recv_idx]);
				}
				...
				auto _durwrt = std::chrono::duration_cast<
								   ClockResolution>(_endwrt - _endcomm)
								   .count();
...
#pragma omp parallel for
				for (int tidx = 0; tidx < curr_cache_per_proc; ++tidx)
				{
					...
					size_t did = bl_strat_id + (rank - 1) * curr_cache_per_proc + tidx;
				...
				for (int tidx = 0; tidx < curr_cache_per_proc; ++tidx)
				{
					...
					LOG_DEBUG("Rank[%d]>> sent dim-exterma 4-doubles to master for did(%d)",
							  rank, bl_strat_id + (rank - 1) * curr_cache_per_proc + tidx);
